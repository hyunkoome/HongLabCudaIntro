...
void RealtimeRaytracing::InitWindow() {
    glfwInit();
...
void RealtimeRaytracing::FramebufferResizeCallback(GLFWwindow *window, int width, int height) {
    auto app = reinterpret_cast<RealtimeRaytracing *>(glfwGetWindowUserPointer(window));
    app->framebufferResized = true;
}

void RealtimeRaytracing::InitVulkan() {
    CreateInstance();
    SetupDebugMessenger();
    CreateSurface();
    PickPhysicalDevice();
...
void RealtimeRaytracing::InitCuda() {
    SetCudaVkDevice();
...
void RealtimeRaytracing::MainLoop() {
...
void RealtimeRaytracing::LoadImageData(const std::string &filename) {
...
void RealtimeRaytracing::CleanupSwapChain() {
...
void RealtimeRaytracing::Cleanup() {
    CleanupSwapChain();
...
void RealtimeRaytracing::RecreateSwapChain() {
...
void RealtimeRaytracing::CreateInstance() {
...
void RealtimeRaytracing::PopulateDebugMessengerCreateInfo(
...
void RealtimeRaytracing::SetupDebugMessenger() {
    if (!enableValidationLayers)
        return;
...
void RealtimeRaytracing::CreateSurface() {
    if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
        throw std::runtime_error("failed to create window surface!");
    }
}

void RealtimeRaytracing::PickPhysicalDevice() {
...
void RealtimeRaytracing::GetKhrExtensionsFn() {
#ifdef _WIN64
...
int RealtimeRaytracing::SetCudaVkDevice() {
...
void RealtimeRaytracing::CreateLogicalDevice() {
...
void RealtimeRaytracing::CreateSwapChain() {
...
void RealtimeRaytracing::CreateImageViews() {
...
void RealtimeRaytracing::CreateRenderPass() {
...
void RealtimeRaytracing::CreateDescriptorSetLayout() {
...
void RealtimeRaytracing::CreateGraphicsPipeline() {
...
void RealtimeRaytracing::CreateFramebuffers() {
...
void RealtimeRaytracing::CreateCommandPool() {
...
void RealtimeRaytracing::CreateTextureImage() {
...
void RealtimeRaytracing::GenerateMipmaps(VkImage image, VkFormat imageFormat) {
...
HANDLE RealtimeRaytracing::GetVkImageMemHandle(
...
HANDLE RealtimeRaytracing::GetVkSemaphoreHandle(
...
#else
int RealtimeRaytracing::GetVkImageMemHandle(
    VkExternalMemoryHandleTypeFlagsKHR externalMemoryHandleType) {
...
int RealtimeRaytracing::GetVkSemaphoreHandle(
    VkExternalSemaphoreHandleTypeFlagBitsKHR externalSemaphoreHandleType, VkSemaphore &semVkCuda) {
...
void RealtimeRaytracing::CreateTextureImageView() {
    textureImageView = CreateImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM);
}

void RealtimeRaytracing::CreateTextureSampler() {
...
VkImageView RealtimeRaytracing::CreateImageView(VkImage image, VkFormat format) {
...
void RealtimeRaytracing::CreateImage(uint32_t width, uint32_t height, VkFormat format,
...
void RealtimeRaytracing::CudaVkImportSemaphore() {
...
void RealtimeRaytracing::CudaVkImportImageMem() {
...
void RealtimeRaytracing::TransitionImageLayout(VkImage image, VkFormat format,
...
void RealtimeRaytracing::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
                                           uint32_t height) {
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void RealtimeRaytracing::CreateVertexBuffer() {
...
void RealtimeRaytracing::CreateIndexBuffer() {
...
void RealtimeRaytracing::CreateUniformBuffers() {
...
void RealtimeRaytracing::CreateDescriptorPool() {
...
void RealtimeRaytracing::CreateDescriptorSets() {
...
void RealtimeRaytracing::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
VkCommandBuffer RealtimeRaytracing::BeginSingleTimeCommands() {
...
void RealtimeRaytracing::EndSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void RealtimeRaytracing::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
uint32_t RealtimeRaytracing::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void RealtimeRaytracing::CreateCommandBuffers() {
...
void RealtimeRaytracing::CreateSyncObjects() {
...
void RealtimeRaytracing::CreateSyncObjectsExt() {
    VkSemaphoreCreateInfo semaphoreInfo = {};
    semaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
...
void RealtimeRaytracing::UpdateUniformBuffer() {
...
void RealtimeRaytracing::DrawFrame() {
...
void RealtimeRaytracing::CudaVkSemaphoreSignal(cudaExternalSemaphore_t &extSemaphore) {
...
void RealtimeRaytracing::CudaVkSemaphoreWait(cudaExternalSemaphore_t &extSemaphore) {
...
void RealtimeRaytracing::SubmitVulkan(uint32_t imageIndex) {
    VkSubmitInfo submitInfo = {};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
...
void RealtimeRaytracing::SubmitVulkanCuda(uint32_t imageIndex) {
    VkSubmitInfo submitInfo = {};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
...
VkShaderModule RealtimeRaytracing::CreateShaderModule(const std::vector<char> &code) {
...
VkSurfaceFormatKHR RealtimeRaytracing::ChooseSwapSurfaceFormat(
...
VkPresentModeKHR RealtimeRaytracing::ChooseSwapPresentMode(
...
VkExtent2D RealtimeRaytracing::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
RealtimeRaytracing::SwapChainSupportDetails
RealtimeRaytracing::QuerySwapChainSupport(VkPhysicalDevice device) {
...
bool RealtimeRaytracing::IsDeviceSuitable(VkPhysicalDevice device) {
...
bool RealtimeRaytracing::CheckDeviceExtensionSupport(VkPhysicalDevice device) {
...
std::vector<const char *> RealtimeRaytracing::GetRequiredExtensions() {
...
bool RealtimeRaytracing::CheckValidationLayerSupport() {
    uint32_t layerCount;
...
std::vector<char> RealtimeRaytracing::ReadFile(const std::string &filename) {
...
VKAPI_ATTR VkBool32 VKAPI_CALL RealtimeRaytracing::debugCallback(
