...
class RealtimeVideoProcessing {
  ...
  private:
    ...
    // 동영상 디코더
    std::unique_ptr<hlab::VideoReader> decoder;
    uint32_t *d_frame = nullptr;  // R8G8B8A8 = 32
    uint32_t *d_frame2 = nullptr; // R8G8B8A8 = 32

    // CUDA objects
    cudaExternalMemory_t cudaExtMemImageBuffer;
    cudaMipmappedArray_t cudaMipmappedImageArray, cudaMipmappedImageArrayTemp,
        cudaMipmappedImageArrayTemp2, cudaMipmappedImageArrayOrig;
    std::vector<cudaSurfaceObject_t> surfaceObjectList, surfaceObjectListTemp,
        surfaceObjectListTemp2;
    cudaSurfaceObject_t *d_surfaceObjectList, *d_surfaceObjectListTemp, *d_surfaceObjectListTemp2;
    ...
    RealtimeVideoProcessing::QueueFamilyIndices FindQueueFamilies(VkPhysicalDevice device) {
        RealtimeVideoProcessing::QueueFamilyIndices indices;
