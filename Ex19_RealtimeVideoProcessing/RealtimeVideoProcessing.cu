...
__global__ void copyFromPixelsKernel(cudaSurfaceObject_t *dstSurfMipMapArray, uint32_t *pixels,
    ...
    if (x < baseWidth && y < baseHeight) {
        ...
        // float4 t = tex2DLod<float4>(textureMipMapInput, float(x) / baseWidth,
        //	float(y) / baseHeight, 0);

        // unsigned int dataB = rgbaFloatToInt(t);
        unsigned int dataB = pixels[baseWidth * y + x];
        surf2Dwrite(dataB, dstSurfMipMapArray[0], 4 * x, y);
...
__global__ void copyFromPixelsKernel2(cudaSurfaceObject_t *dstSurfMipMapArray, uint32_t *pixels,
                                      size_t baseWidth, size_t baseHeight, float th) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (x < baseWidth && y < baseHeight) {
        ...
        float4 c = rgbaIntToFloat(pixels[baseWidth * y + x]);
        const float l = c.x * 0.2126f + c.y * 0.7152f + c.z * 0.0722f; // Luminance

        if (l > th)
            surf2Dwrite(pixels[baseWidth * y + x], dstSurfMipMapArray[0], 4 * x, y);
        else
            surf2Dwrite(0, dstSurfMipMapArray[0], 4 * x, y);
...
__global__ void addKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                          cudaSurfaceObject_t *srcSurfMipMapArray, uint32_t *pixels,
                          size_t baseWidth, size_t baseHeight, float weight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (x < baseWidth && y < baseHeight) {
        ...
        float4 c = rgbaIntToFloat(pix) * weight;

        if (x < boundary) {
            c += rgbaIntToFloat(pixels[baseWidth * y + x]);
        }

        surf2Dwrite(rgbaFloatToInt(c), dstSurfMipMapArray[0], 4 * x, y);
...
extern int filterRadius;
extern int boundary;
extern int boundaryDirection;
extern int filterNumber;
extern bool pauseVideo;
extern bool moveBoundary;

void RealtimeVideoProcessing::CudaUpdateVkImage() {
    CudaVkSemaphoreWait(cudaExtVkUpdateCudaSemaphore);

    if (moveBoundary) {
        boundary += boundaryDirection; // 세로 경계선이 움직이는 효과
        if (boundary > WIDTH - 1) {
            boundary = WIDTH - 1;
    ...
    static int frame = 0;
    if (frame == 0) { // 첫 프레임인 경우, 먼저 읽어들이고 시작 (다른 곳으로 이동 가능)
        cudaMemcpy(d_frame, decoder->GetPixels(), WIDTH * HEIGHT * sizeof(uint32_t),
                   cudaMemcpyHostToDevice);
        copyFromPixelsKernel2<<<dimGrid, dimBlock>>>(d_surfaceObjectListTemp, d_frame, imageWidth,
                                                     imageHeight, 0.8f);
        decoder->Continue();
        decoder->WaitForNextFrame();
    }
    frame += 1;

    if (filterNumber == 1) { // No filter
        cudaMemcpyAsync(d_frame2, decoder->GetPixels(), WIDTH * HEIGHT * sizeof(uint32_t),
                        cudaMemcpyHostToDevice, streamToCopy);
        copyFromPixelsKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(d_surfaceObjectList, d_frame,
                                                                    imageWidth, imageHeight);
        cudaStreamSynchronize(streamToRun); // 이 함수 밖으로 이동 가능
        cudaStreamSynchronize(streamToCopy);
        std::swap(d_surfaceObjectListTemp, d_surfaceObjectListTemp2);
        std::swap(d_frame, d_frame2);
    } else if (filterNumber == 2) { // Sobel
        // TODO:
    } else { // Bloom
        // TODO:
    }
