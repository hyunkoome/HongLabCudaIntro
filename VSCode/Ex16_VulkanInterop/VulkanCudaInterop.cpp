...
void VulkanCudaInterop::InitWindow() {
    glfwInit();
...
void VulkanCudaInterop::FramebufferResizeCallback(GLFWwindow *window, int width, int height) {
    auto app = reinterpret_cast<VulkanCudaInterop *>(glfwGetWindowUserPointer(window));
    app->framebufferResized = true;
}

void VulkanCudaInterop::InitVulkan() {
    CreateInstance();
    SetupDebugMessenger();
    CreateSurface();
    PickPhysicalDevice();
...
void VulkanCudaInterop::InitCuda() {
    SetCudaVkDevice();
...
void VulkanCudaInterop::MainLoop() {
    UpdateUniformBuffer();
...
void VulkanCudaInterop::LoadImageData(const std::string &filename) {
...
void VulkanCudaInterop::CleanupSwapChain() {
...
void VulkanCudaInterop::RecreateSwapChain() {
...
void VulkanCudaInterop::CreateInstance() {
...
void VulkanCudaInterop::PopulateDebugMessengerCreateInfo(
...
void VulkanCudaInterop::SetupDebugMessenger() {
    if (!enableValidationLayers)
        return;
...
void VulkanCudaInterop::CreateSurface() {
    if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
        throw std::runtime_error("failed to create window surface!");
    }
}

void VulkanCudaInterop::PickPhysicalDevice() {
...
void VulkanCudaInterop::GetKhrExtensionsFn() {
#ifdef _WIN64
...
int VulkanCudaInterop::SetCudaVkDevice() {
...
void VulkanCudaInterop::CreateLogicalDevice() {
...
void VulkanCudaInterop::CreateSwapChain() {
...
void VulkanCudaInterop::CreateImageViews() {
...
void VulkanCudaInterop::CreateRenderPass() {
...
void VulkanCudaInterop::CreateDescriptorSetLayout() {
...
void VulkanCudaInterop::CreateGraphicsPipeline() {
...
void VulkanCudaInterop::CreateFramebuffers() {
...
void VulkanCudaInterop::CreateCommandPool() {
...
void VulkanCudaInterop::CreateTextureImage() {
...
void VulkanCudaInterop::GenerateMipmaps(VkImage image, VkFormat imageFormat) {
...
HANDLE VulkanCudaInterop::GetVkImageMemHandle(
...
HANDLE VulkanCudaInterop::GetVkSemaphoreHandle(
...
#else
int VulkanCudaInterop::GetVkImageMemHandle(
    VkExternalMemoryHandleTypeFlagsKHR externalMemoryHandleType) {
...
int VulkanCudaInterop::GetVkSemaphoreHandle(
    VkExternalSemaphoreHandleTypeFlagBitsKHR externalSemaphoreHandleType, VkSemaphore &semVkCuda) {
...
void VulkanCudaInterop::CreateTextureImageView() {
    textureImageView = CreateImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM);
}

void VulkanCudaInterop::CreateTextureSampler() {
...
VkImageView VulkanCudaInterop::CreateImageView(VkImage image, VkFormat format) {
...
void VulkanCudaInterop::CreateImage(uint32_t width, uint32_t height, VkFormat format,
...
void VulkanCudaInterop::CudaVkImportImageMem() {
...
void VulkanCudaInterop::TransitionImageLayout(VkImage image, VkFormat format,
...
void VulkanCudaInterop::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width,
                                          uint32_t height) {
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void VulkanCudaInterop::CreateVertexBuffer() {
...
void VulkanCudaInterop::CreateIndexBuffer() {
...
void VulkanCudaInterop::CreateUniformBuffers() {
...
void VulkanCudaInterop::CreateDescriptorPool() {
...
void VulkanCudaInterop::CreateDescriptorSets() {
...
void VulkanCudaInterop::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
VkCommandBuffer VulkanCudaInterop::BeginSingleTimeCommands() {
...
void VulkanCudaInterop::EndSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void VulkanCudaInterop::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
uint32_t VulkanCudaInterop::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void VulkanCudaInterop::CreateCommandBuffers() {
...
void VulkanCudaInterop::CreateSyncObjects() {
...
void VulkanCudaInterop::CreateSyncObjectsExt() {
    VkSemaphoreCreateInfo semaphoreInfo = {};
    semaphoreInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
...
void VulkanCudaInterop::UpdateUniformBuffer() {
...
void VulkanCudaInterop::DrawFrame() {
...
void VulkanCudaInterop::CudaVkSemaphoreSignal(cudaExternalSemaphore_t &extSemaphore) {
...
void VulkanCudaInterop::CudaVkSemaphoreWait(cudaExternalSemaphore_t &extSemaphore) {
...
void VulkanCudaInterop::SubmitVulkan(uint32_t imageIndex) {
    VkSubmitInfo submitInfo = {};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
...
void VulkanCudaInterop::SubmitVulkanCuda(uint32_t imageIndex) {
    VkSubmitInfo submitInfo = {};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
...
VkShaderModule VulkanCudaInterop::CreateShaderModule(const std::vector<char> &code) {
...
VkSurfaceFormatKHR VulkanCudaInterop::ChooseSwapSurfaceFormat(
...
VkPresentModeKHR VulkanCudaInterop::ChooseSwapPresentMode(
...
VkExtent2D VulkanCudaInterop::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
VulkanCudaInterop::SwapChainSupportDetails
VulkanCudaInterop::QuerySwapChainSupport(VkPhysicalDevice device) {
...
bool VulkanCudaInterop::IsDeviceSuitable(VkPhysicalDevice device) {
...
bool VulkanCudaInterop::CheckDeviceExtensionSupport(VkPhysicalDevice device) {
...
std::vector<const char *> VulkanCudaInterop::GetRequiredExtensions() {
...
bool VulkanCudaInterop::CheckValidationLayerSupport() {
    uint32_t layerCount;
...
std::vector<char> VulkanCudaInterop::ReadFile(const std::string &filename) {
...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanCudaInterop::debugCallback(
